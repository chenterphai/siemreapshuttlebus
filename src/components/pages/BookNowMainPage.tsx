...
const BookNowMainPage = ({
  ...
  preloadedPayments: Preloaded<typeof api.payment.get>;
}) => {
  ...
  const departureFrom = useQuery(api.airportTime.getFrom);
  const departureTo = useQuery(api.airportTime.getTo);

  const [direction, setDirection] = useState(
    JSON.parse(bookingData!)?.direction ?? "from"
  );
  ...
  const [returnTime, setReturnTime] = useState<string | null>(
    JSON.parse(bookingData!)?.returnTime ?? null
  );
  ...
  const [selectedDate, setSelectedDate] = useState<Date | null>(
    JSON.parse(bookingData!)?.departureDate
      ? new Date(JSON.parse(bookingData!)?.departureDate)
      : today
  );
  ...
  const [returnDate, setReturnDate] = useState<Date | null>(
    JSON.parse(bookingData!)?.returnDate
      ? new Date(JSON.parse(bookingData!)?.returnDate)
      : today
  );
  ...
  const [passager, setPassager] = useState(
    JSON.parse(bookingData!)?.passager ?? "1"
  );
    ...
    try {
      ...
      const returnDateData =
        returnDate && trip === "Round trip"
          ? `${moment(returnDate).format("MMM DD, YYYY")} / ${returnTime}`
          : "";
              ...
              <Suspense>
                ...
                <div className="relative lg:col-span-1 col-span-2">
                  <FormControl fullWidth>
                    {departureTo ? (
                      <>
                        <InputLabel id="return-time-select-label">
                          Return Time
                        </InputLabel>
                        <Select
                          ...
                          label="Departure Time"
                          onChange={(e) =>
                            handleChangeReturnTime(e.target.value)
                          }
                        >
                          {departureTo.map((item, index) => (
                            <MenuItem key={index} value={item.time}>
