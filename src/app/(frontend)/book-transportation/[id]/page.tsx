...
export default async function Page({ params }: RequestParams) {
  const { id } = await params;

  const transportationId = id as Id<"transportation">;

  const preloadedTransportation = await preloadQuery(
    api.transportation.select,
    { id: transportationId }
  );
